# backtester.py
# نسخه ۸.۵ - هماهنگ‌سازی با منطق حافظه نقدینگی ربات لایو
...
class Backtester:
    def __init__(self, m15_data, m1_data, h4_data, config):
        ...
        # **FIX**: متغیرهای جدید برای مدیریت حافظه نقدینگی
    ...
    def run(self):
        ...
        # برای جلوگیری از خطا در اندیس‌های اولیه، از یک نقطه مطمئن شروع می‌کنیم
        for i in range(50, len(self.m1_data) - 1):
            ...
            # **FIX**: منطق ریست کردن حافظه در شروع روز جدید
            ...
            # اگر معامله‌ای باز است، فقط شرایط بسته شدن آن را چک کن
            if open_trade:
                if self._check_close_conditions(i, open_trade):
                    open_trade = None # معامله بسته شد
                continue
            
            is_kz, _ = self._is_in_killzone(current_time)

            # اگر خارج از کیل‌زون هستیم، چرخه را ریست کن
            if not is_kz:
    ...
    def _setup_pending_order(self, fvg):
        ...
        # **FIX**: سطح نقدینگی استفاده شده را به حافظه اضافه کن
    ...
    def _check_close_conditions(self, index, trade):
        """شرایط بسته شدن یک معامله باز (SL/TP) را بررسی می‌کند."""
    ...
    def _close_trade(self, index, trade, close_price, reason):
        """یک معامله را می‌بندد و نتایج آن را ثبت می‌کند."""
