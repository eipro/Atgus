# bot/trader.py
# نسخه ۸.۴ - افزودن تابع بستن معاملات در پایان روز و رفع خطا
...
class TradingBot(threading.Thread):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.last_eod_date is not None and now_ny.date() > self.last_eod_date:
                    ...
                    # **FIX**: Reset the memory for each symbol at the start of a new day.
        ...
        if open_positions and len(open_positions) > 0:
            for position in open_positions:
                ...
                request = {
                    ...
                    # **FIX**: Changed filling mode to FOK for better compatibility with brokers.
    ...
    def _execute_trade(self, fvg, symbol):
        ...
        request = {
            ...
            "magic": 12345, "comment": "Argus 8.3", "type_time": mt5.ORDER_TIME_GTC,
            "type_filling": mt5.ORDER_FILLING_FOK,
        }
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            ...
            # **FIX**: Add the used liquidity level to memory upon successful order placement.
    ...
    def _handle_awaiting_sweep(self, symbol):
        ...
        if swept:
            # **FIX**: Check memory before proceeding to avoid re-entry on the same liquidity level.
            if target_level in symbol_state.get('acted_on_liquidity_levels', []):
                # This liquidity has been used before, so we ignore it.
                # A log is not added here to avoid spamming the log file on every tick.
                return
